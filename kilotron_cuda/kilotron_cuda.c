...
void process_perceptron(){
    ...
    float *x = (float*)malloc((2+mydata->communication_num)*sizeof(float));
    x[0]=mydata->molecules_concentration[0];
    x[1]=mydata->molecules_concentration[1];

    for (i = 0;i<mydata->N_Neighbors; i++) {

      for(j=0;j<mydata->communication_num;j++){
    ...
    //printf("%f\n", mydata->prediction);
    for (i = 0;i<mydata->communication_num;i++){
...
void process_message()
{
  ...
  for (i = 0; i < mydata->N_Neighbors; i++)
      ...
      for (c=0;c<mydata->communication_num;c++){
...
void setup() {
    ...
    mydata->communication_num = get_config_communication_chanels("perceptron/NN.txt");

    mydata->communication_chanel=(float*)malloc(mydata->communication_num * sizeof(float));
    int i;
    for (i = 0;i<mydata->communication_num;i++){
